...
static void exam_print_running_test(size_t len, const char* name)
{
    if (!global_env.shortd) {
        ...
        printf("%*.*s: ", -global_env.longest_name_len, global_env.longest_name_len, name);
    }
}
...
static void exam_print_passed_scope(size_t passed, const char* name, double diff)
{
    if (!global_env.shortd) {
        printf("%s[  PASSED  ] %zu test(s) passed in scope %s%s [%.2f s]\n", GREEN, passed, name, NONE, diff);
    }
}
        ...
        for (size_t i = 1; i < argc; ++i) {
            if (strncmp(argv[i], "--list-tests", 12) == 0) {
                ...
                if (strncmp(argv[i], "on", 2) == 0) {
                    ...
                    fprintf(stderr, "second parameter passed to color is neither `on` or `off`\n");
                    free_exam_env();
                    exit(1);
                }
            }
            ...
            else if (strncmp(argv[i], "--version", 8) == 0 || strncmp(argv[i], "-v", 2) == 0) {
                printf("Version 0.1 License MIT (conni2461)\n");
                free_exam_env();
                exit(0);
            }
            else {
                fprintf(stderr, "Option %s not found! See -h for supported options", argv[i]);
                free_exam_env();
                exit(1);
            }
        }
...
int32_t exam_run()
{
    ...
    for (size_t i = 0; i < global_env.tbl.len; ++i) {
        ...
        for (size_t j = 0; j < global_env.tbl.scope[ii].len; ++j) {
            ...
            if (global_env.filter.len > 0 && !exam_filter_test(buf, &global_env.filter)) {
                free(buf);
                continue;
            }
            if (!printed_scope) {
                ...
                // the wrong count
                exam_print_running_test(global_env.tbl.scope[ii].len, global_env.tbl.scope[ii].name);
                printed_scope = true;
            }
        ...
        if (printed_scope) {
            exam_print_passed_scope(scope_passed, global_env.tbl.scope[i].name, diff_scope);
            printf("\n");
        }
        exam_free_shuffle(tests_indices);
    }
...
static void exam_insert_test_in_list(exam_scope_t* list, const char* scope, const char* name, void (*fn)(),
                                     bool pending)
{
    if (list->tests == NULL) {
        list->cap = 16;
    ...
    list->tests[list->len++] = (exam_test_t){.fn = fn, .scope = scope, .name = name, .pending = pending};
}
...
static exam_scope_t* exam_insert_new_scope(const char* scope)
{
    if (global_env.tbl.len + 1 >= global_env.tbl.cap) {
        ...
        global_env.tbl.scope = realloc(global_env.tbl.scope, sizeof(exam_scope_t) * global_env.tbl.cap);
    }

    global_env.tbl.scope[global_env.tbl.len] =
        (exam_scope_t){.tests = NULL, .len = 0, .cap = 0, .name = scope, .before = NULL, .after = NULL};
...
void _exam_register_test(const char* scope, const char* name, void (*fn)(), bool pending)
{
    ...
    for (size_t i = 0; i < global_env.tbl.len; ++i) {
        if (strcmp(scope, global_env.tbl.scope[i].name) == 0) {
            exam_insert_test_in_list(&global_env.tbl.scope[i], scope, name, fn, pending);
            return;
        }
    }
    exam_insert_test_in_list(exam_insert_new_scope(scope), scope, name, fn, pending);
}
...
void _exam_assert_true(bool value, const char* file, int32_t line)
{
    if (!value) {
        if (!global_env.shortd) {
            ...
            printf(" [ true != false ] ");
            printf("%s%s [ %strue%s != %sfalse%s ] ", RED, NONE, GREEN, NONE, RED, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_false(bool value, const char* file, int32_t line)
{
    if (value) {
        if (!global_env.shortd) {
            ...
            printf(" [ false != true ] ");
            printf("%s%s [ %sfalse%s != %strue%s ] ", RED, NONE, GREEN, NONE, RED, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_double(double expected, double result, const char* file, int32_t line)
{
    if (fabs(expected - result) >= EPSILON) {
        if (!global_env.shortd) {
            printf("  Error at line: %s:%d\n", file, line);
            printf("  %sExpected: %f %sResult: %f%s\n", GREEN, expected, RED, result, NONE);
        }
        else {
            printf("%s%s [ %s%f%s != %s%f%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_int(int32_t expected, int32_t result, const char* file, int32_t line)
{
    if (expected != result) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%d%s != %s%d%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_uint(uint32_t expected, uint32_t result, const char* file, int32_t line)
{
    if (expected != result) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%u%s != %s%u%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_long(long int expected, long int result, const char* file, int32_t line)
{
    if (expected != result) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%ld%s != %s%ld%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_ulong(unsigned long int expected, unsigned long int result, const char* file, int32_t line)
{
    if (expected != result) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%lu%s != %s%lu%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_long_long(long long expected, long long result, const char* file, int32_t line)
{
    if (expected != result) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%lld%s != %s%lld%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
                                   ...
                                   int32_t line)
{
    if (expected != result) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%llu%s != %s%llu%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_char(char expected, char result, const char* file, int32_t line)
{
    if (expected != result) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%c%s != %s%c%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_str(const char* expected, const char* result, const char* file, int32_t line)
{
    if (strcmp(expected, result) != 0) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%s%s != %s%s%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_equal_ptr(void* expected, void* result, const char* file, int32_t line)
{
    if (expected != result) {
        if (!global_env.shortd) {
            ...
            printf("%s%s [ %s%p%s != %s%p%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
        longjmp(global_sig, 1);
    }
}
...
void _exam_assert_not_equal_long(long int expected, long int result, const char* file, int32_t line)
{
    if (expected == result) {
        if (!global_env.shortd) {
            printf("  Error at line: %s:%d\n", file, line);
            printf("  %sExpected \"%ld\" and result \"%ld\" should not be the same%s\n", RED, expected, result, NONE);
        }
        else {
            printf("%s%s [ %s%ld%s == %s%ld%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
...
void _exam_assert_not_equal_ulong(unsigned long int expected, unsigned long int result, const char* file, int32_t line)
{
    if (expected == result) {
        if (!global_env.shortd) {
            printf("  Error at line: %s:%d\n", file, line);
            printf("  %sExpected \"%lu\" and result \"%lu\" should not be the same%s\n", RED, expected, result, NONE);
        }
        else {
            printf("%s%s [ %s%lu%s == %s%lu%s ] ", RED, NONE, GREEN, expected, NONE, RED, result, NONE);
        }
